...
pub fn solution() {
    ...
    for game in games {
        let current_cube = max_game_cubes(game);
        power_sum += current_cube.green * current_cube.red * current_cube.blue;
    }
...
pub fn max_game_cubes(game: &str) -> Cube {
    ...
    // track the current cube count for the current hand
    let mut potential_max_cubes: Cube = Cube {
        red: 0,
        blue: 0,
        green: 0,
    };

    // split the hands into their cube amounts
    for hand in hands {
        let cubes = hand.split(", ");
        for cube in cubes {
            let items = cube.split(" ").collect::<Vec<&str>>();
            let color = items[1];
            let num = items[0].parse::<i32>().expect(format!("Unable to parse {} as a number", items[0]).as_str());
            match color {
                "blue" => potential_max_cubes.blue = potential_max_cubes.blue.max(num),
                "red" => potential_max_cubes.red = potential_max_cubes.red.max(num),
                "green" => potential_max_cubes.green = potential_max_cubes.green.max(num),
                _ => {},
            };
        }
    }
...
Notes:
- I think I am already getting this info?
*/